    ...
    while(True):
        try:
            ...
            for item in foundWillmessages:
                ...
                #=========
                # description:
                # intori roye VMN ... ya VCN ... hasas mishavad!!!
                # va mohem nist ke shomareye sensoresh chande
                # =========


                # Define the pattern to match keys
                pattern = '*'+item.split('/')[2]+'*'

                # Retrieve the list of keys that match the pattern
                matchedkeys = r.keys(pattern)

                # print('matchedkeys === '+str(len(matchedkeys)))

                # print(matchedkeys[0].decode())


                # print(item.split('/')[2])
                # foundNOTWILLmessage === VMN'
                foundNOTWILLmessage=[key.decode() for key in matchedkeys if 'Will' not in key.decode()]  # hameye Will ha

                # print('len_foundNOTWILLmessage', '  '+str(len(foundNOTWILLmessage)))
                # if ('VMN' in item):   # Will messages only for VMN
                #     foundNOTWILLmessage=[key.decode() for key in all_keys if re.match(item.split('/')[2],key.decode()) and 'Will' not in key.decode()]  # hameye Will ha
                ...
                # if (len(foundNOTWILLmessage)>0):
                #     print('all_keys_matche=== '+'ooo')
                #     all_keys_matche=list()
                #     for notWill in foundNOTWILLmessage:
                #         print(str(notWill))
                #         all_keys_matche.append(r.keys(notWill))

                #     all_keys_matche=set(all_keys_matche)

                #     print('all_keys_matche=== '+all_keys_matche[0])
                ...
                if (len(foundNOTWILLmessage)>0):
                    ...
                    if (maxTimestamp>time_will):
                        logging.info('++++++++not send Will   '+ foundNOTWILLmessage[0]+'   '+str(datetime.now()))
                        ...
                        if (datetime.now()-time_will).seconds>DELAY:  # wait until DELAY (send Will message frequently)
                            ...
                            logging.info('--------send Will message  (Delay '+ str((datetime.now()-time_will).seconds)+') ' + item.replace('/Will','/Will_0')+'   '+str(datetime.now()))                           
                            r.delete(item)
